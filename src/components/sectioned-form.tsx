...
import { type SectionProps, type WorkflowProps } from '@/queries/useWorkflowQuery';
import { useState } from 'react';
import { Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Button, buttonVariants } from '@/components/ui/button';
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from '@/components/ui/collapsible';
import { ArrowLeft, ChevronRight, ChevronDown, Edit, Save, Check } from 'lucide-react';
import { cn } from '@/lib/utils';
import FormField from '@/components/form-field';

export function SectionedForm({ data, disabled, isSubmitting }: { data: WorkflowProps; disabled?: boolean; isSubmitting?: boolean }) {
                ...
                <Card className='max-w-4xl mx-auto pt-0 md:pt-0 overflow-hidden'>
                    ...
                    <CardContent>
                        <div className="flex flex-col md:flex-row flex-wrap gap-4 justify-between">
                            {form.fields.slice().sort((a, b) => a.position - b.position).map((field) => <FormField key={field.id} {...field} disabled={disabled} />)}
                        </div>
                    </CardContent>
                    <CardFooter>
                        <Button type='button' className='w-full' disabled={disabled || isSubmitting}>
                            {isSubmitting ? (
                                <>
            ...
            <div className="h-full flex flex-col">
                <div className="bg-primary/5 border-b border-border p-4 md:p-6 mb-6">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <Button type='button' variant="ghost" size="sm" onClick={() => setActiveForm('')}>
                    ...
                    {data.sections.slice().sort((a, b) => a.position - b.position).slice().sort((a, b) => a.position - b.position).map((section) => (
                        <Card key={section.slug} className="overflow-hidden gap-2 lg:gap-2 pb-3 md:pb-3">
                            ...
                            <CardFooter className='flex items-center justify-between px-3 md:px-4'>
                                ...
                                <Button type='button' size='sm' className='w-32' disabled={disabled || isSubmitting}>
                                    {isSubmitting ? (
                                        <>
                                            <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                                            Approving...
