...
export function NavigationSidebar({ collapsed = false }: NavigationSidebarProps) {
    const [openGroups, setOpenGroups] = useState<Set<string>>(new Set(['land-uses']));
    ...
    const renderNavigationItem = (item: NavigationItem, group?: string) => {
        ...
        const buttonContent = (
            <Link
                key={item.id}
                className={cn(buttonVariants({ variant: isActive ? "default" : "ghost" }), `w-full ${collapsed ? 'px-2 justify-center' : group ? 'justify-start px-4' : 'justify-start px-3'} h-8 ${isActive
            ...
            return (
                <TooltipProvider key={item.id}>
                    <Tooltip>
                        ...
                        <TooltipContent side="right" className="flex items-center gap-2 text-white">
                            {item.label}
                            {item.badge && (
    ...
    const renderNavigationGroup = (group: NavigationGroup) => {
        ...
        const hasActiveItem = group.items.some(item => pathname.includes(item.id));
                ...
                <TooltipProvider key={group.id}>
                    <Tooltip>
                        ...
                        <TooltipContent side="right" className="p-0 max-w-60 text-white">
                            <div className="p-2">
                                <div className="font-medium text-sm mb-2 ml-3">{group.label}</div>
                                <div className="space-y-1">
                                    {group.items.map(item => (
                                        <Link
                                            key={item.id}
                                            className={cn(buttonVariants({ variant: pathname.includes(`${group.id}/${item.id}`) ? "default" : "ghost", size: "sm" }), `w-full justify-start h-8 ${pathname.includes(item.id)
    ...
    return (
        <nav className="p-3 space-y-1">
            {page && page?.module ? renderNavigationItem({
                id: page.module,
                label: "Dashboard",
                icon: <Home className="h-4 w-4" />,
            }) : null}
