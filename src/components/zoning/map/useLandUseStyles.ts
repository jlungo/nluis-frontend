...
type HatchParams = { fg: string; bg?: string | null; angle?: number; width?: number; spacing?: number; };
type DotsParams = { fg: string; bg?: string | null; size?: number; spacing?: number; };
...
function drawDotsPattern(canvas: HTMLCanvasElement, p: DotsParams) {
  ...
  const W = spacing; canvas.width = W; canvas.height = W;
  const ctx = canvas.getContext("2d")!;
  if (p.bg) { ctx.fillStyle = p.bg; ctx.fillRect(0, 0, W, W); }
  ctx.fillStyle = p.fg; ctx.beginPath(); ctx.arc(W / 2, W / 2, size, 0, Math.PI * 2); ctx.fill();
}

function drawImagePattern(canvas: HTMLCanvasElement, url: string, bg?: string | null, onReady?: () => void) {
  const ctx = canvas.getContext("2d")!;
  const W = 64; canvas.width = W; canvas.height = W;
  if (bg) { ctx.fillStyle = bg; ctx.fillRect(0, 0, W, W); }
  const img = new Image(); img.crossOrigin = "anonymous"; img.src = url;
  img.onload = () => { ctx.drawImage(img, 0, 0, W, W); onReady?.(); };
}
...
export function parseLandUseStyles(landUses: LandUseDto[], map?: mapboxgl.Map) {
  ...
  const asHex = (c?: string | null) => (typeof c === "string" && c.startsWith("#") ? c : undefined);

  for (const lu of landUses) {
    const s = lu.style;
    if (!s || !Array.isArray(s.layers)) { if (lu.color) solidColorByLU.set(lu.id, lu.color); continue; }
    ...
    if (!poly || !poly.fill) { if (lu.color) solidColorByLU.set(lu.id, lu.color); continue; }
    const fill = poly.fill;
    if (fill.type === "solid" && asHex(fill.color)) { solidColorByLU.set(lu.id, fill.color); continue; }
    if (fill.type === "pattern") {
      if (fill.pattern === "hatch") {
        const p: HatchParams = { fg: fill.fg || "#000", bg: fill.bg || null, angle: Number(fill.angle ?? 45), width: Number(fill.width ?? 1), spacing: Number(fill.spacing ?? 8) };
        ...
        const p: DotsParams = { fg: fill.fg || "#000", bg: fill.bg || null, size: Number(fill.size ?? 2), spacing: Number(fill.spacing ?? 6) };
