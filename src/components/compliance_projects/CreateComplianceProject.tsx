...
import { MapPin, Calendar, DollarSign, FileText, X, ChevronDown, ChevronRight, AlertCircle } from 'lucide-react';
...
interface ComplianceProjectData extends CreateProjectDataI {
...
export default function CreateComplianceProject(props: Props) {
  const { user } = useAuth();
...
function Forms({ moduleLevel, redirectPath = '/compliance', localities, funders, project, organizationId }: FormProps) {
  ...
  const [formData, setFormData] = useState<ComplianceProjectData>({
    name: project?.name || '',
  ...
  const getTargetLevel = (): string => {
    switch (moduleLevel) {
      ...
      case LOCALITY_LEVELS.COMPLIANCE:
  ...
  const handleInputChange = (key: keyof ComplianceProjectData, value: any) => {
  ...
  const isFormValid = (): boolean => {
    const { name, description, registration_date, authorization_date, budget, funder_ids, compliance_type, monitoring_frequency, enforcement_level } = formData;

    const basicFieldsValid = !!(name && description && registration_date && authorization_date && budget && funder_ids.length > 0 && compliance_type && monitoring_frequency && enforcement_level);
    ...
    // For COMPLIANCE and MNE (which allow all levels), require locality selection
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Failed to create compliance project:", error);
    }
  };

  const renderLocalitySelection = () => {
    // For COMPLIANCE and MNE, show locality selection dialog (not fixed National coverage)
    // Only show "National Coverage" message for NATIONAL level specifically
      ...
      return (
        <div className="flex items-center gap-2 p-4 bg-blue-50 rounded-lg border border-blue-200 dark:bg-blue-900/50 dark:border-blue-800">
          ...
          <div>
            ...
            <p className="text-sm text-blue-700 dark:text-blue-300">This compliance project will cover the entire nation of Tanzania.</p>
          </div>
        </div>
      );
    }
      ...
      <div className="space-y-4">
        <Dialog open={isLocalityModalOpen} onOpenChange={setIsLocalityModalOpen}>
          ...
          <DialogContent aria-describedby='SetlocalityDialog' className="w-full transition-all sm:max-w-[95vw] lg:max-w-3xl h-[80vh] flex flex-col">
            <DialogHeader>
              <DialogTitle>Select Localities for Compliance Monitoring</DialogTitle>
            </DialogHeader>
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <form onSubmit={handleSubmit} className="space-y-6">
        {/* Info Banner */}
        <div className="flex items-start gap-3 p-4 bg-amber-50 rounded-lg border border-amber-200 dark:bg-amber-900/30 dark:border-amber-800">
          <AlertCircle className="h-5 w-5 text-amber-600 dark:text-amber-500 flex-shrink-0 mt-0.5" />
          <div>
            <h3 className="font-medium text-amber-900 dark:text-amber-100">Compliance Monitoring Project</h3>
            <p className="text-sm text-amber-800 dark:text-amber-200 mt-1">
              This project type is designed for environmental, regulatory, and social compliance tracking and enforcement.
            </p>
          </div>
        </div>
        ...
        <Card className='pt-0 md:pt-0 overflow-hidden'>
          ...
          <CardContent className="space-y-4">
            <FormFieldInput
              ...
              required
              placeholder="Enter compliance project name"
            />
            <FormFieldInput
              type="text"
              id="reference-number"
            ...
            <FormFieldInput
              ...
              required
              placeholder="Describe the compliance objectives and scope"
            />
          </CardContent>
        </Card>
        ...
        <Card className='pt-0 md:pt-0 overflow-hidden'>
          ...
          <CardContent className="space-y-4">
            <FormFieldInput
              ...
              value={formData.compliance_type || 'environmental'}
              values={[formData.compliance_type || 'environmental']}
              onChange={(val: string) => handleInputChange('compliance_type', val as 'environmental' | 'regulatory' | 'social')}
              onValuesChange={(vals: (string | number)[]) => handleInputChange('compliance_type', vals[0] as 'environmental' | 'regulatory' | 'social')}
              required
              options={[
            ...
            <FormFieldInput
              ...
              value={formData.monitoring_frequency || 'quarterly'}
              values={[formData.monitoring_frequency || 'quarterly']}
              onChange={(val: string) => handleInputChange('monitoring_frequency', val as 'monthly' | 'quarterly' | 'semi-annual' | 'annual')}
              onValuesChange={(vals: (string | number)[]) => handleInputChange('monitoring_frequency', vals[0] as 'monthly' | 'quarterly' | 'semi-annual' | 'annual')}
              required
              options={[
            ...
            <FormFieldInput
              ...
              value={formData.enforcement_level || 'medium'}
              values={[formData.enforcement_level || 'medium']}
              onChange={(val: string) => handleInputChange('enforcement_level', val as 'low' | 'medium' | 'high')}
              onValuesChange={(vals: (string | number)[]) => handleInputChange('enforcement_level', vals[0] as 'low' | 'medium' | 'high')}
              required
              options={[
                { value: 'low', label: 'Low' },
        ...
        <div className="flex justify-end gap-4 -mt-1 pb-4">
          ...
          >
            {project ? (isPending || isPendingCreate ? 'Updating Project...' : 'Update Project') : (isPending || isPendingCreate ? 'Creating Project...' : 'Create Compliance Project')}
          </Button>
        </div>
      </form>
    </div>
