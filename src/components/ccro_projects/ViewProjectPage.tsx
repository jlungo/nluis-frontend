...
import { useDeleteCCROProject as useDeleteProject, useCCROProjectQuery as useProjectQuery } from '@/queries/useCCROProjectQuery';
...
import { PlanStatusColumn } from '@/components/project/plan-status-column';
...
const ButtonsComponent: React.FC<{ moduleLevel: string, project: ProjectI, approval_status: number, approval_status_atleast_one: number }> = ({ moduleLevel, project, approval_status, approval_status_atleast_one }) => {
  ...
  const handleDelete = () => {
    try {
      ...
      toast.promise(mutateAsyncDelete(project.id), {
        ...
        success: () => {
          setOpenDelete(false)
          navigate(`/ccro-management/${moduleLevel}`, { replace: true })
  ...
  <Link to={`/ccro-management/${moduleLevel}/${project.id}/edit`} className={cn(buttonVariants({ size: 'sm' }), "gap-2 w-fit")}>
...
const CoverageAreasCard: React.FC<{ project: ProjectI }> = ({ project }) => {
  ...
  const localities = project?.localities ? statusFilter
    ? project.localities.filter((loc: any) => loc.approval_status === statusFilter)
    : project.localities : []
  ...
  return (
    <Card className='shadow-none pt-0 md:pt-0 overflow-hidden'>
      <CardHeader className="border-b pt-5 md:pt-6 [.border-b]:pb-4 md:[.border-b]:pb-4 bg-accent dark:bg-input/30">
        <CardTitle className="flex items-center justify-between">
          ...
          {project.localities && project.localities.length > 0 && (
            <MapDialog
              title={project.name}
              overlayMapsIds={project?.localities.map((loc: any) => loc.locality__id)}
            />
          )}
        </CardTitle>
      </CardHeader>
      <CardContent>
        {project.localities && project.localities.length > 0 ?
          <DataTable
            columns={[...LocalityColumns, PlanStatusColumn]}
            data={localities}
            ...
            rowActions={(locality) => {
              ...
              const checkPlan = async () => {
                setIsChecking(true);
                const loadingToast = toast.loading('Checking plan requirements...');
                
                try {
                  console.log('Checking plan for locality:', locality.locality__id); // Debug log
                  const response = await api.get(`/zoning/plans/?locality=${locality.locality__id}`);
                  console.log('Plan check response:', response.data); // Debug log
                  
                  toast.dismiss(loadingToast);
                  const hasPlan = response.data?.length > 0;
                  
                  if (!hasPlan) {
                    toast.info(`No land use plan found for ${locality.locality__name}. Please upload one to proceed.`);
                  }
                  
                  return hasPlan;
                } catch (error) {
                  console.error('Plan check error:', error); // Debug log
                  toast.dismiss(loadingToast);
                  toast.error('Failed to check plan requirements');
                  return false;
                } finally {
                  setIsChecking(false);
                }
              ...
              return (
                <>
                  <Button
                    type="button"
                    size="sm"
                    className="disabled:opacity-10 mr-5 bg-primary/20 text-primary hover:bg-primary/30 dark:text-primary flex items-center gap-2"
                    disabled={locality.approval_status !== 2 || isChecking}
                    onClick={async () => {
                      const hasPlan = await checkPlan();
                      if (hasPlan) {
                        navigate(`${locality.id}/workflow`);
                      } else {
                        setShowPlanUpload(true);
                      }
                    }}
                  >
                  ...
                  <PlanUpload
                    isOpen={showPlanUpload}
                    onOpenChange={setShowPlanUpload}
                    localityId={locality.locality__id}
                    localityName={locality.locality__name}
                    onSuccess={() => {
