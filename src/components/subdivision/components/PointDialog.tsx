...
export default function PointDialog({ isOpen, onOpenChange, onConfirm }: PointDialogProps) {
  ...
  const parseCsv = useCallback(async (file: File) => {
    ...
    if (!out.length) {
      toast.error("CSV not recognized. Expect 3 columns: order, coordinate A, coordinate B.");
      return;
    }
  ...
  const updateRow = (idx: number, field: keyof PointRow, value: string) => {
    setPointRows((rows) => {
      const copy = [...rows];
      const val = value.trim() === "" ? "" : Number(value);
  ...
  const removeRow = (idx: number) => {
    setPointRows((rows) => (rows.length <= 1 ? rows : rows.filter((_, i) => i !== idx)));
  };

  const handleConfirm = () => {
    onConfirm(pointRows, coordMode, coordMode === "projected" ? epsgCode : undefined);
    onOpenChange(false);
  };
    ...
    >
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Enter coordinates to add points to the map. You can choose between WGS84 (latitude/longitude) or UTM coordinate systems.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Expected columns: <strong>order</strong>,{" "}
