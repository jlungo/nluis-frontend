...
const ButtonsComponent: React.FC<{ module?: ModuleTypes, moduleLevel: string, project: ProjectI, approval_status: number, approval_status_atleast_one: number }> = ({ module, moduleLevel, project, approval_status, approval_status_atleast_one }) => {
  ...
  const getNavigatePath = () => {
    if (module) {
      return `/monitoring-and-evaluation/${module}/${moduleLevel}`;
    } else {
      return `/monitoring-and-evaluation/${moduleLevel}`;
    }
  };

  // Build edit path based on whether module exists
  const getEditPath = () => {
    if (module) {
      return `/monitoring-and-evaluation/${module}/${moduleLevel}/${project.id}/edit`;
    } else {
      return `/monitoring-and-evaluation/${moduleLevel}/${project.id}/edit`;
    }
  };
  ...
  return (
    <div className='flex gap-2 flex-col md:flex-row items-end'>
      ...
      {canEditProject(user.role.name, approval_status_atleast_one) ?
        <Link to={getEditPath()} className={cn(buttonVariants({ size: 'sm' }), "gap-2 w-fit")}>
