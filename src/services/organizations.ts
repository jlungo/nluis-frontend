...
export const organizationService = {
  ...
  getOrganizations: async (filters?: OrganizationFilters) => {
    const response = await api.get<PaginatedResponseI<OrganizationI>>(Modal, {
      params: filters,
    });
  ...
  createOrganizationType: async (data: Partial<OrganizationType>) => {
    const response = await api.post<OrganizationType>(
      `${Modal}/organization-types/create/`,
      data
    );
    return response.data;
  },
  ...
  partialUpdateOrganizationType: async (
    id: string,
    data: PatchedOrganizationType
  ) => {
    const response = await api.patch<OrganizationType>(
      `${Modal}/organization-types/${id}/`,
      data
    );
    return response.data;
