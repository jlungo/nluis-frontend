...
interface QuestionnairePreviewTesterProps {
    questionnaireData: QuestionnaireTemplate;
    onSave: (questionnaireData: QuestionnaireTemplate) => void;
    onEdit: () => void;
}
...
export function FormPreviewTester({
    ...
    onSave,
    onEdit,
}: QuestionnairePreviewTesterProps) {
    ...
    const expandAllSections = () => {
        ...
        if (questionnaireData?.sections) {
            questionnaireData.sections.filter(section => section.is_active === true).forEach(section => {
    ...
    const renderFormPreview = () => (
        <div className="space-y-6">
            <div className="text-center space-y-2">
                <h2 className="text-lg md:text-2xl font-semibold">{questionnaireData.name}</h2>
                {questionnaireData.description && (
                    <p className="text-muted-foreground">{questionnaireData.description}</p>
                )}
                <div className="flex items-center justify-center gap-2">
                    {/* <Badge variant="outline" className='capitalize'>{questionnaireData.type}</Badge> */}
                    <Badge variant="outline">{questionnaireData.module}</Badge>
                </div>
            </div>
            ...
            <div className="flex items-center justify-between border-b pb-4">
                <div className="flex flex-col md:flex-row items-center gap-2">
                    ...
                    <Badge variant="outline" className="text-xs">
                        {questionnaireData?.sections ? `${questionnaireData.sections.filter(section => section.is_active === true).length} sections` : null}
                    </Badge>
                </div>
            ...
            <div className="space-y-4">
                {questionnaireData?.sections && questionnaireData.sections.filter(section => section.is_active === true).sort((a, b) => a.order - b.order).map((section) => (
                    <Card key={section.id}>
                        <Collapsible
    ...
    const renderFormTest = () => (
        <div className="space-y-6">
            <div className="flex items-end justify-between gap-1">
                <div>
                    <h2 className="text-sm sm:text-base md:text-xl font-semibold">{questionnaireData.name}</h2>
                    {questionnaireData.description && (
                        <p className="text-xs md:text-sm lg:text-base text-muted-foreground">{questionnaireData.description}</p>
                    )}
                </div>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <pre className="text-xs bg-muted p-4 rounded-lg overflow-auto max-h-96">
                                {JSON.stringify(questionnaireData, null, 2) || 'No questionnaire data available'}
                            </pre>
                        </CardContent>
                    </Card>
                </TabsContent>
    ...
    return (
        <div className={`${isFullscreen ? 'fixed inset-0 z-[9999] overflow-y-auto bg-background' : 'min-h-screen'}`}>
            <div className="max-w-6xl mx-auto p-6 space-y-6">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-lg md:text-2xl font-semibold">Questionnaire Preview & Testing</h1>
                        <p className="text-xs md:text-sm xl:text-base text-muted-foreground">
                            Preview your form questionnaire structure and test its functionality before saving
                        </p>
                    </div>
                    <div className="flex flex-col md:flex-row items-center gap-2">
                        ...
                        <Button size="sm" onClick={() => onSave(questionnaireData)} className="gap-2">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
                            ...
                            <div className="text-center">
                                <div className="text-lg lg:text-2xl font-semibold text-primary flex items-center justify-center gap-2">
                                    ...
                                    {questionnaireData?.sections && questionnaireData.sections.filter(section => section.is_active === true).reduce((count, section) =>
                            ...
                            <div className="text-center">
                                <div className="text-lg md:text-2xl font-semibold text-primary flex items-center justify-center gap-2">
                                    ...
                                    {questionnaireData?.sections ? questionnaireData.sections.filter(section => section.is_active === true).length : 0}
                                </div>
                            ...
                            <div className="text-center">
                                <div className="text-lg md:text-2xl font-semibold text-primary flex items-center justify-center gap-2">
                                    ...
                                    {Math.round((Object.keys(formValues).length / (questionnaireData.sections!.filter(section => section.is_active === true).reduce((count, section) =>
                                        count + section.forms.filter(form => form.is_active === true).reduce((subCount, form) =>
